/**
 * Trackcrastinate - Supabase Module
 * 
                    ...
                    single: async function() {
                      ...
                      try {
                        ...
                        if (table === 'users') {
                          ...
                          const key = data.id || `${data.userId}_${data.date}`;
...
async function getTrackedSites(userId) {
  try {
    ...
    return data && data.trackedSites ? data.trackedSites : DEFAULT_TRACKED_SITES;
  } catch (error) {
...
/**
 * Get a user's time entries for a specific day
 * @param {string} userId - The user's ID
...
async function getDailySummary(userId, date) {
  try {
    ...
    entries.forEach(entry => {
      ...
      if (entry.domain) {
        siteBreakdown[entry.domain] = (siteBreakdown[entry.domain] || 0) + duration;
      }
      
      // Update category breakdown
      if (entry.category) {
        categoryBreakdown[entry.category] = (categoryBreakdown[entry.category] || 0) + duration;
      }
    });
...
/**
 * Get top time-wasting sites for a user
 * @param {string} userId - The user's ID
 * @param {string} date - The date in YYYY-MM-DD format
 * @param {number} maxResults - Maximum number of sites to return
 * @returns {Promise<Array>} - Array of top sites with time spent
 */
async function getTopSites(userId, date, maxResults = 5) {
  try {
    const summary = await getDailySummary(userId, date);
    
    if (!summary || !summary.siteBreakdown) {
      return [];
    }
    
    // Convert to array and sort
    const sites = Object.entries(summary.siteBreakdown)
 ...
 * @param {string} userId - The user's ID
 * @param {string} date - The date in YYYY-MM-DD format
 * @returns {Promise<Array>} - Array of categories with time spent
 */
async function getCategoryBreakdown(userId, date) {
  try {
    const summary = await getDailySummary(userId, date);
...
self.supabaseModule.app = supabase;
self.supabaseModule.auth = supabase.auth;
self.supabaseModule.db = supabase;
self.supabaseModule.usersCollection = usersCollection;
self.supabaseModule.timeEntriesCollection = timeEntriesCollection;
self.supabaseModule.dailySummariesCollection = dailySummariesCollection;
self.supabaseModule.DEFAULT_USER_SETTINGS = DEFAULT_USER_SETTINGS;
self.supabaseModule.DEFAULT_TRACKED_SITES = DEFAULT_TRACKED_SITES;

// User data functions
self.supabaseModule.getUserSettings = getUserSettings;
self.supabaseModule.saveUserSettings = saveUserSettings;
self.supabaseModule.getTrackedSites = getTrackedSites;
self.supabaseModule.saveTrackedSites = saveTrackedSites;

// Time tracking functions
self.supabaseModule.saveTimeEntry = saveTimeEntry;
self.supabaseModule.getTimeEntriesForDay = getTimeEntriesForDay;
self.supabaseModule.getDailySummary = getDailySummary;
self.supabaseModule.getTopSites = getTopSites;
self.supabaseModule.getCategoryBreakdown = getCategoryBreakdown;
